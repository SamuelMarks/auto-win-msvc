...
TEST test_sys_file_flock(void) {
  ...
  _sopen_s(&fd, "test_flock.txt", _O_CREAT | _O_RDWR, _SH_DENYNO,
           _S_IREAD | _S_IWRITE);
#else
  ...
  /* Wait, since it's the SAME handle, LockFileEx doesn't block the same handle
     from locking again? Actually, LockFileEx on the same handle will fail with
     ERROR_LOCK_VIOLATION if overlapping. */
...
SUITE(sys_file_suite) { RUN_TEST(test_sys_file_flock); }
