...
int getopt_long(int argc, char *const argv[], const char *optstring, const struct option *longopts, int *longindex) {
    ...
    if (argv[optind][1] == '-' && argv[optind][2] != '\0') {
        ...
        for (i = 0; longopts[i].name != NULL; i++) {
            if (strncmp(name, longopts[i].name, namelen) == 0 && longopts[i].name[namelen] == '\0') {
                ...
                if (longopts[i].has_arg == required_argument) {
                    ...
                    if (has_eq) {
                        if (opterr) fprintf(stderr, "option '--%s' doesn't allow an argument\n", longopts[i].name);
                        return '?';
                    }
                    optarg = NULL;
                }
        ...
        if (opterr) fprintf(stderr, "unrecognized option '%s'\n", argv[optind]);
        optopt = '?';
        optind++;
        return '?';
    }
