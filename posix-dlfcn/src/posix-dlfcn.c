...
static int set_dlerror(DWORD err_code) {
  ...
  if (FormatMessageA(FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS,
                     ...
                     thread_dlerror_msg,
                     (DWORD)(sizeof(thread_dlerror_msg) - 1), NULL) == 0) {
/* Fallback if FormatMessage fails */
    ...
    while (len > 0 && (thread_dlerror_msg[len - 1] == '\r' ||
                       thread_dlerror_msg[len - 1] == '\n')) {
...
int dlclose(void *handle) {
  ...
  /* POSIX dlclose on the main program handle generally succeeds without
   * actually freeing it */
...
/* RTLD_NEXT is not easily supported in a generic cross-platform way on Windows
 * without DbgHelp */
#if defined(__STDC_SECURE_LIB__) || (defined(_MSC_VER) && _MSC_VER >= 1400)
  ...
  /* Standard trick to avoid strict C89 warnings when casting function to void
   * pointer */
  ret_ptr = NULL;
#if defined(__STDC_SECURE_LIB__) || (defined(_MSC_VER) && _MSC_VER >= 1400)
...
int dladdr(const void *addr, Dl_info *info) {
  ...
  len = GetModuleFileNameA(handle, thread_dladdr_fname,
                           (DWORD)sizeof(thread_dladdr_fname));
  ...
  /* Resolving the nearest symbol name/address would require dbghelp.dll or
     parsing PE exports. Standard fallback is NULL. */
  info->dli_sname = NULL;
  info->dli_saddr = NULL;
...
int dummy_posix_dlfcn(void) { return 0; }
