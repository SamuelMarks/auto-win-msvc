...
typedef int BOOL;
typedef void *HANDLE;
__declspec(dllimport) HANDLE WINAPI OpenProcess(DWORD dwDesiredAccess,
                                                BOOL bInheritHandle,
                                                DWORD dwProcessId);
__declspec(dllimport) DWORD WINAPI WaitForSingleObject(HANDLE hHandle,
...
TEST test_posix_spawn_file_actions_badf(void) {
  ...
  ASSERT_EQ(EBADF,
            posix_spawn_file_actions_addopen(&actions, -1, "test.txt", 0, 0));
...
TEST test_posix_spawn_execution(void) {
  ...
  ASSERT_EQ(0, posix_spawn(&pid, "C:\\Windows\\System32\\cmd.exe", &actions,
                           &attr, argv, envp));
  if (pid > 0) {
    HANDLE hProcess =
        OpenProcess(PROCESS_QUERY_INFORMATION | SYNCHRONIZE, FALSE, pid);
